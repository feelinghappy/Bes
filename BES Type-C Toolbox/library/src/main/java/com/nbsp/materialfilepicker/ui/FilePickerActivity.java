...
public class FilePickerActivity extends AppCompatActivity implements DirectoryFragment.FileClickListener {
    ...
    public static final String ARG_FORLDER = "arg_is_folder";
    ...
    private void initArguments() {
        if(getIntent().hasExtra(ARG_FORLDER)){
            isFolder = getIntent().getBooleanExtra(ARG_FORLDER ,false);
        }
    ...
    @Override
    public void onFileSelect(File selectFile) {
        if(selectFile != null){
            choosePath = selectFile.getPath();
        }
    }
...
//            setResultAndFinish(clickedFile.getPath());
    ...
    public void onClickButton(View view){
        if(choosePath != null){
            setResultAndFinish(choosePath);
        }else{
            if(isFolder){
                Toast.makeText(this , "还没有选择升级文件夹",Toast.LENGTH_SHORT).show();
            }else{
                Toast.makeText(this , "还没有选择升级文件",Toast.LENGTH_SHORT).show();
            }
        }
    }
