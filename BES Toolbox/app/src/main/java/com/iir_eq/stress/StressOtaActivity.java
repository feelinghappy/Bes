...
public abstract class StressOtaActivity extends BaseActivity implements ConnectCallback,View.OnClickListener {
    ...
    private static final int MSG_OTA_REPEAT = 0x04 ;
	private static final int MSG_UPDATE_RESULT_INFO = 0x05;
    ...
    protected long castTime ;//ota cast time
    protected long sendMsgFailCount = 0 ;//TEMP DATA
    ...
    protected void repeatOTA(){
        LOG(TAG, "repeatOTA()");
        removeTimeout();
        mState = STATE_IDLE;
        LogUtils.writeForOTAStatic(TAG , "repeatOTA  ");
        Message message = mMsgHandler.obtainMessage(MSG_OTA_REPEAT);
        mMsgHandler.sendMessageDelayed(message, 10000);
    }
            ...
            if (mSupportNewOtaProfile || mWritten) {
                ...
                if ((mOtaPacketItemCount < mOtaData[mOtaPacketCount].length)) {
                    ...
                    if (!sendRet) {
                        ...
                        mOtaPacketItemCount++;
                        if (mOtaPacketItemCount == mOtaData[mOtaPacketCount].length) {
                            sendTimeout(R.string.ota_time_out, CMD_DISCONNECT, 90000);   //RESEND
                        }else{
